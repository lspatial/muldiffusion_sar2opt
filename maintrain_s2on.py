...
import torch
from guided_diffusion.train_test import TrainLoop
...
def configArgs():
    ...
    parser.add_argument('--data_path', type=str,
                        default='/devb/WHU-OPT-SAR/jointpatch224full_txt', help='data file path')
    parser.add_argument('--map_swinunet', type=str,
                        default=None, help='Trained SWIN-Unet file: default: no trained UNet')
    parser.add_argument('--diffusion_trained_model', type=str,
                        default=None , #'/tmp/openai-2025-06-01-11-35-29-722585/model050400.pt',
                        help='Trained diffusion Unet file: default: no trained model')
    #conditions config
    parser.add_argument('--condition_way', type=str,
                        default='SARMAP', help='condition way:{SAR, SARMAP, SARMAPTXT,SARTXT} default: SAR')
    ...
    parser.add_argument('--output_root', type=str,
                        default='/devb/sar2opt_diff/sar2opt_map_texttest_Oct3',
    ...
    parser.add_argument('--gpu', type=int, default=1, help='Gpu index. (Default 0)')
    ...
    elif args.condition_way == 'SARMAP' or args.condition_way == 'SARMAPTXT':
        config['num_weighted_channels'] = 3
        config['con_weight'] = 0.5
        config['in_channels'] += 4
    if args.condition_way == 'SARMAPTXT' or args.condition_way == 'SARTXT':
        config['use_text_crossattention']=True
    else :
        config['use_text_crossattention']=False
...
def main():
    ...
    if os.path.exists(args.output_root):
        ...
        confirmation = input(f"Directory '{args.output_root}' already exists. Delete it? (y/[N]): ")
        if confirmation.upper() == 'Y':
            # User confirmed, delete the directory
            print(f"Deleting directory: {args.output_root}")
            shutil.rmtree(args.output_root)
        else:
            # User did not confirm, exit the program
    ...
    train_loader, val_loader = create_dataloaders(args.data_path , channels=3, batch_size=args.batch_size,
                            num_workers=10, apply_augmentation=False, val_split=0.2, subset_size=args.subset,
                            resize_to=None,crop_size=None,label2text=True)
    ...
    mtrainer.run_train(train_loader,val_loader,args.map_swinunet,num_epochs=args.num_epochs,device=device)
